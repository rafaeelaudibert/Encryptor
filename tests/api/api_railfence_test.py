...
def test_railfence_encrypt_api(client):
    response = client.get("/api/railfence/encrypt/helloworld?rail_height=2").get_json()
...
def test_railfence_encrypt_api_invalid_rail_height(client):
    response = client.get(
        "/api/railfence/encrypt/helloworld?rail_height=two"
    ).get_json()
    assert response["status"] == 400


def test_railfence_decrypt_api(client):
    response = client.get("/api/railfence/decrypt/hloolelwrd?rail_height=2").get_json()
    assert response["status"] == 200
...
def test_railfence_decrypt_api_invalid_rail_height(client):
    response = client.get(
        "/api/railfence/decrypt/helloworld?rail_height=two"
    ).get_json()
    assert response["status"] == 400
