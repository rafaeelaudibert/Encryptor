...
class Vigenere:
    ...
    """
    square or Vigenère table, also known as the tabula recta,
    ...
    def encrypt(text: str, key: str = KEY, tabula_recta=None):
        ...
        if key.isalpha():
            ...
            encrypt_message = [
                (value[0] + value[1]) % len(Vigenere.ALPHABET)
    ...
    def _encode(word: str):
        return [
            key
            for char in word
            for key, value in Vigenere.ALPHABET.items()
            if char == value
        ]
    ...
    def _comparator(message: list, key: list):
        return dict(
            [(i, [char[0], char[1]]) for i, char in enumerate(zip(message, cycle(key)))]
        )
